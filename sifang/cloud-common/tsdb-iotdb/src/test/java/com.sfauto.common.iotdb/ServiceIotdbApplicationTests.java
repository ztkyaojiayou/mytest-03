package com.sfauto.common.iotdb;

import com.sfauto.common.iotdb.service.JDBCService;
...
@SpringBootTest
class ServiceIotdbApplicationTests {
    ...
    private static ClusterInfoService.Client client;

    @Test
    public void testJdbc() {
        try{
            Class.forName(IOTDB_DRIVER);
            String connectStr = String.format("jdbc:iotdb://%s:%s?version=V_0_13", IOTDB_URL, IOTDB_PORT);
            try (Connection connection = DriverManager.getConnection(connectStr, IOTDB_USERNAME, IOTDB_PASSWORD); Statement statement = connection.createStatement()) {
                ...
                for (int index = 0; index <= 100; index++) {
                    statement.addBatch(prepareInsertStatment(index));
                }
...
//                outputResult(resultSet);
                ResultSet resultSet = statement.executeQuery("select ** from root.sg1.d2");
                outputResult(resultSet);
            } catch (IoTDBSQLException e) {
                System.out.println(e.getMessage());
            }
...
//    search();
    ...
    public void testCluster() {
        try {
            transport = RpcTransportFactory.INSTANCE.getTransport(new TSocket(IOTDB_URL, IOTDB_PORT));
            String host = IoTDBDescriptor.getInstance().getConfig().getRpcAddress();
            System.out.println(host);
//            Integer port = ClusterDescriptor.getInstance().getConfig().getClusterRpcPort();
//            transport = RpcTransportFactory.INSTANCE.getTransport(new TSocket(host, port));
        ...
        try {
            List<Node> rings = client.getRing();
            for (Node ring : rings) {
                System.out.println(ring);
            }
            Map<Node, Integer> allNodeStatus = client.getAllNodeStatus();
            System.out.println(allNodeStatus);
            String instrumentingInfo = client.getInstrumentingInfo();
            System.out.println(instrumentingInfo);
        } catch (TException e) {
    ...
    private static String prepareInsertStatment(int time) {
        ...
        return "insert into root.sg1.d2(timestamp, s1) values(" + time + "," + random.nextInt(10000) + ")";
//        return "insert into root.sg1.d1(timestamp, s1, s2, s3) values(" + time + "," + random.nextLong(10000) + "," + random.nextLong(10000) + "," + random.nextLong(10000) + ")";
    }
    ...
    private static boolean search(){
        ...
        try {
            ...
            sessionEnableRedirect = new Session(IOTDB_URL, 6667, "root", "root");
...
//        System.out.println(dataSet.getColumnNames());
                ...
                System.out.println("表数量：\t" + dataSet.getColumnNames().size());
            }
        }
        catch (Exception e) {
            String msg = e.getMessage();
        }
        Duration duration = Duration.between(inst1, Instant.now());
        System.out.println("以毫秒计的时间差：\t"+duration.toMillis());
        System.out.println("以秒计的时间差：\t"+duration.getSeconds());
...
//            createMultiTimeseries(); //创建综合时间序列
//            insertRecord(); //插入记录
            insertTablet(); //插入时间片
            //    insertTabletWithNullValues(); //插入时间片及空值数据
            //    insertTablets(); //插入时间片
            //    insertRecords(); //插入记录

            //    selectInto(); //选择插入
            //    createAndDropContinuousQueries(); //创建并删除持久队列
            //    nonQuery(); //无返回值查询
            //    query(); //查询
            //    queryWithTimeout(); //带超时时间的查询
            //    rawDataQuery(); //数据查询
            //    lastDataQuery(); //末尾数据查询
            //    queryByIterator(); //集合查询
            //    deleteData(); //删除数据
...
//    }
    ...
    private static void setTimeout() throws StatementExecutionException {
        Session tempSession = new Session(IOTDB_URL, 6667, "root", "root", 10000, 20000);
