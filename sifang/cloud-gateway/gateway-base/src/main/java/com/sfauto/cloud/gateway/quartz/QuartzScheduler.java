...
public class QuartzScheduler {
    ...
    public void startJob(String cron, String jobName) throws SchedulerException {
        startJob1(scheduler, cron, jobName);
        //startJob2(scheduler);
        scheduler.start();
    }
    ...
    private void startJob1(Scheduler scheduler, String cron, String jobName) throws SchedulerException {
        // 通过JobBuilder构建JobDetail实例，JobDetail规定只能是实现Job接口的实例
        // JobDetail 是具体Job实例
        JobDetail jobDetail = JobBuilder.newJob(RealDataJob.class).withIdentity(jobName, jobName).build();
        // 基于表达式构建触发器
        CronScheduleBuilder cronScheduleBuilder = CronScheduleBuilder.cronSchedule(cron);
        // CronTrigger表达式触发器 继承于Trigger
        // TriggerBuilder 用于构建触发器实例
        CronTrigger cronTrigger = TriggerBuilder.newTrigger().withIdentity(jobName, jobName)
                .withSchedule(cronScheduleBuilder).build();
